...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    [Header("Movement")]
    public Vector3 hookMoveMomentum;
    ...
    void Start()
    {
        ...
        hookComponent = GetComponent<GrapplingHook>();
    ...
    void Update()
    {
        ...
        if (hookComponent.IsHookMoving)
        {
            isGrounded = false;
        }
        else if (!hookComponent.IsHookMoving)
        {
            GroundCheck();
        }       
        ...
        bool isSprinting = m_InputHandler.GetSprintInputHeld();
        {
            ...
            else
            {
                if (wallRunComponent == null || (wallRunComponent != null && !wallRunComponent.IsWallRunning()))
                {
                    ...
                    //apply momentum
                    characterVelocity += hookMoveMomentum;

                    //damp momentum
                    if (hookMoveMomentum.magnitude>0f)
                    {
                        float momentumDrag = 10f;
                        hookMoveMomentum -= hookMoveMomentum * momentumDrag * Time.deltaTime;
                        if (hookMoveMomentum.magnitude<0f)
                        {
                            hookMoveMomentum = Vector3.zero;
                        }
                    }

                    // limit air speed to a maximum, but only horizontally
                    // float verticalVelocity = characterVelocity.y;
                    float verticalVelocity = Mathf.Min(characterVelocity.y, 14f);
